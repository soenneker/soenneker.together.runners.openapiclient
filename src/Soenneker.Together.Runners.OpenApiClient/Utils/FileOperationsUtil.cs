...
public sealed class FileOperationsUtil : IFileOperationsUtil
{
    ...
    public async ValueTask Process(CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(filePath))
            throw new Exception("Together OpenAPI download did not produce a file path.");
        ...
        await _kiotaUtil.Generate(fixedFilePath, "TogetherOpenApiClient", Constants.Library, gitDirectory, cancellationToken).NoSync();
    ...
    public async ValueTask DeleteAllExceptCsproj(string directoryPath, CancellationToken cancellationToken = default)
    {
        if (!(await _directoryUtil.Exists(directoryPath, cancellationToken)))
            throw new DirectoryNotFoundException($"The generated-client source directory does not exist: {directoryPath}");
    ...
    private async ValueTask BuildAndPush(string gitDirectory, CancellationToken cancellationToken)
    {
        ...
        if (!successful)
            throw new InvalidOperationException("The generated Together OpenAPI client did not build successfully.");
